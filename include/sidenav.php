<?php
  ...
  require_once '../Database/connection.php'; // make sure this is present where needed

  $stmt = $conn->prepare("
    SELECT COALESCE(up.full_name, u.username) AS display_name
    FROM users u
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .sidebar {
    ...
    overflow-y: auto; box-shadow: 2px 0 8px rgba(0,0,0,0.2); padding-top: 1rem;
  }
  ...
  .nav-icon { width: 30px; display: flex; justify-content: center; font-size: 1.2rem; }
  .nav-label { margin-left: 12px; white-space: nowrap; }
  .sidebar.collapsed .nav-label { display: none; }

  .toggle-btn {
    position: fixed; top: 1rem; left: 80px; z-index: 1001;
    background: #fff; border: none; border-radius: 50%;
    padding: .5rem .7rem; cursor: pointer; box-shadow: 0 0 6px rgba(0,0,0,0.15);
    transition: var(--transition);
  }
  .sidebar:not(.collapsed) ~ .toggle-btn { left: calc(var(--sidebar-width) + 10px); }
</style>
...
<!-- /.  
  ...
  <a class="nav-link disabled" style="margin-top:auto; cursor:default; opacity:0.7; pointer-events:none;">
